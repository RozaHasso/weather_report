        ...
        window.init = function () {
            ...
            request.onload = () => {
                ...
                pre.forEach(function (item) {
                    ...
                    var obj = tmppre[item.time.split('T', 1)] = tmppre[item.time.split('T', 1)] || { total: 0, type: item.type, unit: item.unit, time: item.time, place: item.place }
                    obj.total += item.value
                });

                const finalfortotalpre = Object.entries(tmppre).map(function (entry) {
                    return { value: entry[1].total, type: entry[1].type, unit: entry[1].unit, time: entry[0], place: entry[1].place }
                })
                ...
                request.onload = () => {
                    ...
                    request.onload = () => {
                        ...
                        wind.forEach(function (item) {
                            // if property for current date already exists  we update existing otherwise start new one
                            var obj = tmpwind[item.time.split('T', 1)] = tmpwind[item.time.split('T', 1)] || { count: 0, total: 0, type: item.type, unit: item.unit, time: item.time, place: item.place }
                            obj.count++
                            obj.total += item.value
                        });
